...
class Program
{
    static void Main()
    {
        ...
        if (!Input.ValidateChosenNumberOfFile(chosenNumberOfFileAsKey, dictionaryOfDirectoryFiles)) throw new Exception("Invalid number of file");
        var pathOfChosenFile = dictionaryOfDirectoryFiles[chosenNumberOfFileAsKey];
        
        var fileExtension = TemporaryName.GetFileExtension(pathOfChosenFile);
        ...
        if (!Input.ValidateNumberOfLines(tempStoreForLinesToPrint))  throw new Exception("Invalid number of lines to skip");
        var numberOfLinesToPrint = int.Parse(tempStoreForLinesToPrint);  

        var readLines = Parser.GetLinesFromFile(pathOfChosenFile, out var propertyNames, fileExtension, numberOfLinesToPrint, numberOfLinesToSkip);
        ...
        {
            if (!Input.ValidateChosenColumns(tempColumnsToDisplay, propertyNames)) throw new Exception("Invalid names of columns");
